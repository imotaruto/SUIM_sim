...
class DefineEventDrivenApp < ANL::ANLApp
  ...
  attr_accessor :write_hit_output # WriteHitTree の出力ファイル名を追加
  ...
  def setup()
    ...
    chain :WriteHitTree
    chain :SaveData
    with_parameters(output: @write_hit_output)
...
# メインスクリプト部分
time_start = 0.0
input_file = "check_fulltime_no_photoabs.root"
...
a2.write_hit_output = "#{filename_base}_hits.root" # WriteHitTree の出力ファイル名を設定
